...
LightFrustumDebugPass::LightFrustumDebugPass(GUID passGuid) 
	: RenderPassBase(passGuid){}

LightFrustumDebugPass::~LightFrustumDebugPass()
{

}

// Interface
bool LightFrustumDebugPass::Initialize(GraphicsContext* graphicsContext, ShaderManager* shaderManager)
{
	if (m_initialized)
	{
		return true;
	...
	if (!m_graphicsPipelineState->Initialize(graphicsContext, shaderManager, ShaderType::LIGHT_SHAFT_PRE_PASS_ROOT_SIGNATURE, ShaderType::LIGHT_SHAFT_PRE_PASS_VERTEX_SHADER, ShaderType::SHADER_TYPE_NONE, ShaderType::LIGHT_FRUSTUM_RENDER_PASS_PIXEL_SHADER))
	{
		return false;
	}
...
bool LightFrustumDebugPass::PopulateCommands(World* world, GraphicsContext* graphicsContext)
{
	...
	if (!dependencyEarlyZPass || !dependencyLightingPass)
	{
		return false;
	}
	...
	UINT64 rtvId = dependencyLightingPass->GetRenderTargetBufferRtvId();
	D3D12_CPU_DESCRIPTOR_HANDLE rtvHandle;
	D3D12_CPU_DESCRIPTOR_HANDLE dsvHandle;
	descHeapManager->GetRenderTargetViewCpuHandle(rtvId, rtvHandle);
	...
	ResourceBarrierTransition(m_pDepthResource.Get(), commandList, D3D12_RESOURCE_STATE_COPY_DEST);
	dependencyEarlyZPass->DepthBufferBarrierTransition(commandList, D3D12_RESOURCE_STATE_COPY_SOURCE);
	commandList->CopyResource(m_pDepthResource.Get(), dependencyEarlyZPass->GetDepthBufferResource());
	ResourceBarrierTransition(m_pDepthResource.Get(), commandList, D3D12_RESOURCE_STATE_DEPTH_WRITE);
